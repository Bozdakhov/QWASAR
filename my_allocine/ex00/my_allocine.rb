require 'sqlite3'
db = SQLite3::Database.new('movies.db')   # Connect to the database
requests = {}    # Your SQL requests
...
requests["Find the actors with all information who played a role in the movies 'Annie Hall'"] = "SELECT * FROM actors WHERE id IN (SELECT act_id FROM movies_actors WHERE mov_id = (SELECT id FROM movies WHERE mov_title = 'Annie Hall'));"
...
requests["Find all the actors who have not acted in any movie between 1990 and 2000 (select only actor first name, last name, movie title and release year)"] = "DELETE from actors"
